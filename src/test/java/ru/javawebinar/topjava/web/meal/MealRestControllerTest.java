...
class MealRestControllerTest extends AbstractControllerTest {
    ...
    void getBetween() throws Exception {
        LocalDateTime start = LocalDateTime.parse("2015-05-30T10:00:00");
        LocalDateTime finish = LocalDateTime.parse("2015-05-30T23:00:00");
        List<Meal> expected = Arrays.asList(MEAL3, MEAL2, MEAL1);
        mockMvc.perform(MockMvcRequestBuilders.get(REST_MEAL_URL))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON));
        assertMatch(mealService.getBetweenDates(start.toLocalDate(), finish.toLocalDate(), USER_ID), expected);
    }
}